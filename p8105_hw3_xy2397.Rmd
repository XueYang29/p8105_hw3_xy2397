...
The dataset "Instacart" contains online grocery orders from instacart users. The size of the dataset is `r nrow(instacart)` rows x `r (ncol(instacart))` columns, which means that it contains `r nrow(instacart)` records of online orders about `r (ncol(instacart))` variables. There are some key variables such as id (order_id, product_id, user_id, aisle_id, department_id) for identifier, order_number for the order sequence number for this user, order_dow for the day of the week on which the order was placed, order_hour_of_day for the hour of the day on which the order was placed and product_name for the name of product.
